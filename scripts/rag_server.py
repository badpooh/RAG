...
def call_ollama(prompt, model="cyankiwi/Qwen3.5-4B-AWQ-4bit"):
    """단일 프롬프트 호출"""
    resp = http_requests.post("http://10.10.26.101:8000/v1/chat/completions", json={
        "model": model,
...
def call_ollama_batch(prompts, model="cyankiwi/Qwen3.5-4B-AWQ-4bit"):
    """여러 프롬프트를 동시에 보내고 결과를 순서대로 반환"""
...
""")
                ...
                # 병렬 호출
                step1_results = call_ollama_batch(step1_prompts, args.llm_model)
