...
SYNONYMS = {
    ...
    "무효전력량": ["reactive energy", "VARh"],
    "피상전력": ["apparent power", "S", "VA"],
    ...
    "THR": ["thermal relay", "열동계전기", "과부하계전기", "thermal overload"],
    "LSR": ["locked rotor", "구속전류", "기동실패", "locked rotor relay"],
    "LOCK": ["locked rotor", "구속", "rotor lock"],
    "RESTART": ["restart inhibit", "재기동금지", "재기동 금지", "재투입금지"],
    ...
    "BREAKER FAILURE": ["차단기 실패", "BF relay", "BF", "차단실패"],
    "FUSE FAILURE": ["퓨즈 단선", "fuse blown", "FF", "퓨즈 고장"],
    "CT SUPERVISION": ["CT 감시", "CT 단선감시", "ct open"],
    "TRIP COIL SUPERVISION": ["트립코일 감시", "trip coil", "TC supervision"],
...
def expand_query(q):
    ...
    for keyword, synonyms in SYNONYMS.items():
        if keyword.lower().replace(" ", "") in q_lower:
            expanded += " " + " ".join(synonyms)
        else:
            for syn in synonyms:
                if syn.lower().replace(" ", "") in q_lower:
                    expanded += " " + keyword + " " + " ".join(synonyms)
                    break
    return expanded
...
def call_ollama(prompt, model="qwen3.5:4b"):
    """단일 프롬프트 호출 (Ollama) — 취소 가능 (스트리밍)"""
...
def search(
    ...
    # 동의어 확장
    search_q = expand_query(q)
    if search_q != q:
        print(f"  [Expand] 원본: {q} → 확장: {search_q}")
    ...
    for r in results:
        ...
        # BugUrl 추출
        bugurl_match = re.search(r'BugUrl:\s*(https?://\S+)', text)
        bugurl = bugurl_match.group(1).strip() if bugurl_match else "-"
        
        # BugUrl 제거한 텍스트
        clean_text = re.sub(r'BugUrl:\s*https?://\S+', '', text)
...
"""
                ...
                for batch_start in range(0, len(retrieval_results), BATCH_SIZE):
                    ...
                    step1_prompt = f"""당신은 문서 분류 전문가입니다.
...
[관련 문서 번호]
"""
                    batch_result = call_ollama(step1_prompt, args.llm_model).strip()
                    ...
                    valid_ranks = {r['rank'] for r in batch}
                    batch_nums = [n for n in batch_nums if n in valid_ranks]
                    all_selected_nums.extend(batch_nums)
                    print(f"  [Step1 Batch {batch_start//BATCH_SIZE + 1}] {len(batch)}건 중 {len(batch_nums)}건 선별")
...
[관련 문서 ID]
"""
                    s2_result = call_ollama(s2_prompt, args.llm_model).strip()
...
[표 행]
"""
                        batch_answer = call_ollama(batch_prompt, args.llm_model).strip()
                        if batch_answer:
